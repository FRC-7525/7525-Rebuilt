...
public final class ShooterMath {
    ...
    public static Optional<ShotSolution> solveHubShot(
            Pose2d robotPose,
    ...
    public static Optional<ShotSolution> solveAllianceShot(
            Pose2d robotPose,
            Translation2d robotVelocity) {
     ...
     */
    private static Optional<ShotSolution> solveMovingShot(
            Pose2d robotPose,
            Pose2d targetPose,
            Translation2d robotVelocity,
            List<ShotSample> table) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            double newDistance =
                    baseDistance - vParallel * sol.timeOfFlightSeconds;
    ...
    private static ShotSolution interpolateByDistance(
        ...
        double t = (distanceMeters - lower.distanceMeters)
                 / (upper.distanceMeters - lower.distanceMeters);

        return new ShotSolution(
                Degrees.of(lerp(
                        lower.hoodAngle.in(Degrees),
                        upper.hoodAngle.in(Degrees),
                        t)),
                RotationsPerSecond.of(lerp(
                        lower.flywheelSpeed.in(RotationsPerSecond),
                        upper.flywheelSpeed.in(RotationsPerSecond),
                        t)),
    ...
    private static double clampToTable(double distance, List<ShotSample> table) {
        return Math.max(
                table.get(0).distanceMeters,
                Math.min(distance, table.get(table.size() - 1).distanceMeters)
        );
    }
